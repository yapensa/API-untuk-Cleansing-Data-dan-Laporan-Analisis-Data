...
def perbaiki_kalimat(huruf):
    # parameter huruf akan mengalami preprocessing, yaitu menghapus karakter yang tidak diinginkan seperti:
    # username, URL, dan karakter spesial serta menggabungkan kembali hasilnya menjadi sebuah string yang terdiri 
    # dari kata-kata yang dipisahkan oleh spasi.
    # ' '.join(re.sub("(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)"," ",huruf).split())
    ...
    # menghapus seluruh karakter ':' yang terdapat dalam string huruf
    huruf = re.sub(r':', '', huruf)
